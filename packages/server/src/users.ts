...
export async function authenticate(
  ...
  if (user.passwordHash === null) {
    ...
    if (claimed.rowsAffected === 1) return { id: user.id, email: user.email, passwordHash }
    ...
    const now = await db.select().from(users).where(eq(users.id, user.id)).get()
 ...
 */
export async function findCredential(db: Db, id: number): Promise<Credential | null> {
...
export function createAccount(db: Db, email: string): Promise<Account> {
