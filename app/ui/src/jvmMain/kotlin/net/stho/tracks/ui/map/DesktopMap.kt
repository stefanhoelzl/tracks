 ...
 *
 * [rewriteUrl] may send a request somewhere else — the screenshot tests serve their tiles locally — by returning the
 * new URL, or leave it alone by returning null.
 */
fun configureDesktopMap(rewriteUrl: (String) -> String? = { null }) {
    DefaultMapRuntime.configure(
        MapRuntimeOptions(requestInterceptor = MapRequestInterceptor(rewriteUrl = { rewriteUrl(it.url) })),
    )
}
